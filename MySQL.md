...
- `-v` — подключить к контейнеру внешнюю папку\
	...
	- `/path/to/my.cnf:/etc/mysql/conf.d/my.cnf` - дополнительно можно указать к внешнему к файлу конфигурации сервера.
...
Выдача прав пользователю `user01` на выполнение Select для БД `my_database`
```sql
grant select on my_database.* to 'user01'@'%';
```
...
См. также оператор Describe для получение метаданных о таблице.
...
# Производительность
## Получение плана выполнения запроса - Explain;
```sql
explain <запрос>
```
Показывает информацию о плане выполнения запроса. Сам запрос не выполняется.

Пример
```sql
MySQL [employees]> explain select * from employees where last_name = 'Codd';
...
- `id`: Идентификатор строки. Каждая строка имеет уникальный идентификатор.
- `select_type`: Тип выборки. В данном случае это SIMPLE, что означает простой SELECT (не используется с UNION или подзапросами).
- `table`: Имя таблицы.
- `type`: Тип соединения. ALL означает полный скан таблицы.
- `possible_keys`: Список возможных индексов, которые могут быть использованы в этом запросе.
- `key`: Индекс, который MySQL решил использовать для этого запроса. В данном случае индекс не используется (NULL).
- `key_len`: Длина ключа, который MySQL решил использовать для этого запроса. В данном случае ключ не используется (NULL).
- `ref`: Столбцы или константы, которые используются с ключом.
- `rows`: Оценка количества строк, которые будут обработаны при выполнении запроса.
- `filtered`: оценочный процент строк таблицы, которые будут отфильтрованы условием запроса.
...
```
После изменения файла конфигурации сервер нужно перезапустить.

# Резервное копирование, экспорт и импорт

`mysqldump` - программа для резервного копирования БД MySQL. Она должна запускаться от имени пользователя БД. Рекомендуется создавать отдельного для этих целей. Пользователю нужны как минимум привилегии select для копирования таблиц. И дополнительные привилегии для выгрузки представлений (views), триггеров.
...
Можно настроить автоматическое резервное копирование запуская mysqldump по расписанию, например через программу cron.

Такое резервное копирование может быть медленным, если БД большая. Физическое копирование файлов БД может быть эффективнее в таких случаях.

См. также репликацию БД - автоматическое копирование данных на аналогичный сервер СУБД.
...
**Выполнить sql файл из mysql shell**
```sql
source my_sql_file.sql
```

Хотя из mysql shell можно запускать обычные консольные программы и выполнять комманды (`system <cmd>`), но операция `cd` не изменяет текущего рабочего каталога.
...
show variables;
```
Имена переменных можно фильтровать, как в любом select запросе. Например, можно показать только значения переменных, имена которых содержат слово `global`:
```sql
show variable like '%general%';
```

Показать значения переменной:
```sql
...
Включить логирование в таблицу
```sql
SET global general_log = 1; 
SET global log_output = 'table';
-- SET global log_output = 'file';
```
Теперь все запросы будут записываться в таблицу `mysql.general_log`:
```
+----------------------------+----------------------------+-----------+-----------+--------------+---------------------------------+
| event_time                 | user_host                  | thread_id | server_id | command_type | argument                        |
+----------------------------+----------------------------+-----------+-----------+--------------+---------------------------------+
| 2024-03-24 14:38:32.417760 | root[root] @  [172.12.23.34] |        21 |         1 | Query        | select * from mysql.general_log |
```
