...
Выдача прав пользователю `user01` на выполнеие Select для БД `my_database`
```sql
...
# Производительность
## Получение плана выполнеия запроса - Explain;
```sql
explain <запрос>
```
Показывает информацию о планевыполнения запроса. Сам запрос не выполняется.
